...
- (void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)data {
    //DLog(@"got %d bytes", data.length);
    [self.container appendData:data];
}

- (void)connectionDidFinishLoading:(NSURLConnection *)connection {
    //DLog(@"got all data, root = %@", self.jsonRootObject);
    //NSString *allData = [[NSString alloc] initWithData:self.container encoding:NSUTF8StringEncoding];
    //DLog(@"recevied %@", allData);
