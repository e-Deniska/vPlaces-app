...
NSString * const VPLHueConnectionDidFailNotificaiton = @"VPLHueConnectionDidFailNotificaiton";
...
- (id)initWithHueHost:(NSString *)host clientType:(NSString*)clientType username:(NSString *)username {
    self = [super init];
    if (self) {
        self.deviceType = clientType;
...
- (void)registerUsernameWithCompletion:(void (^)(DPHue *, NSError *))block
{
...
{
    if ([d respondsToSelector:@selector(objectAtIndexedSubscript:)])
    {
        NSString *username = d[0][@"success"][@"username"];
    ...
    if (![d respondsToSelector:@selector(objectForKeyedSubscript:)]) {
        ...
        _authenticated = NO;
        DLog(@"array instead of dict = %@", d);
//        [[NSNotificationCenter defaultCenter] postNotificationName:VPLHueConnectionDidFailNotificaiton
//                                                            object:self];
        return;
    }
    //WSLog(@"got response: %@", d);
